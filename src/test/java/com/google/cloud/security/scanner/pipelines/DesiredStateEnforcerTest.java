...
public class DesiredStateEnforcerTest {
  ...
  private String constructMessage(GCPResource resource, Map<StateSource, GCPResourceState> input) {
    ...
    return "Live state for resource " + resource.toString()+ " changed from:\n" +
        liveState.toString() + "to the desired state:\n" + desiredState.toString();
  }

  private void setUpSetIamPolicy(final Policy[] policy) {
    try {
      doAnswer(new Answer<SetIamPolicy>() {
        @Override
        public SetIamPolicy answer(InvocationOnMock invocationOnMock) throws Throwable {
          SetIamPolicyRequest request = (SetIamPolicyRequest) invocationOnMock.getArguments()[1];
          policy[0] = request.getPolicy();
          return mock(SetIamPolicy.class);
        }
      }).when(this.projectsObject).setIamPolicy(anyString(), any(SetIamPolicyRequest.class));
    } catch (IOException e) {
      fail("Could not set up setIamPolicy");
    }
  }
